...
$stmt = $dbcon->prepare("SELECT COUNT(*) FROM helpdesk_request WHERE user_id = {$user_id}");
$stmt->execute();
...
$sql = "SELECT A.id AS request_id,A.status as request_status,A.text,B.name as service_name,
...
foreach ($result as $row) {
  if (in_array($row['request_status'], [1, 2])) {
    $status = "<a href='/helpdesk/view/{$row['request_id']}' class='badge text-bg-{$row['status_color']} fw-lighter'>{$row['status_name']}</a>";
  } else {
    $status = "<a href='/helpdesk/complete/{$row['request_id']}' class='badge text-bg-{$row['status_color']} fw-lighter'>{$row['status_name']}</a>";
  }
