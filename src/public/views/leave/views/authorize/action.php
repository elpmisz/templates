...
if ($action === "add") :
  try {
    ...
    if ($count > 0) {
      $Validation->alert("danger", "ข้อมูลซ้ำในระบบ กรุณาตรวจสอบข้อมูล", "/leave/authorize");
    }

    $Authorize->add([$type_id, $user_id]);

    $Validation->alert("success", "เพิ่มข้อมูลเรียบร้อย", "/leave/authorize");
...
if ($action === "delete") :
  try {
    ...
    $Validation->alert("success", "ดำเนินการเรียบร้อย", "/leave/authorize");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
endif;
