...
class Authorize
{
  ...
  {
    $sql = "INSERT INTO helpdesk_authorize(type_id,user_id) VALUES(?,?)";
  ...
  {
    $sql = "SELECT COUNT(*) FROM helpdesk_authorize WHERE type_id = ? AND user_id = ? AND status = 1";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute($data);
  ...
  {
    $sql = "UPDATE helpdesk_authorize SET
    status = 2,
    updated = NOW()
    WHERE id = ?";
    $stmt = $this->dbcon->prepare($sql);
